...
- **ProtoKrum** — our proposed two-gate aggregation defense:
  ...
  - **Gate 2 (parameter surface):** amplified Krum scoring applied to the
    remaining clients.
  - `ProtoKrum.py` also includes the **ProtoTrimmed** variant — prototypical
    networks combined with trimmed mean aggregation — used as a robust
...
├── ProtoKrum.py                 # proposed defense (includes ProtoTrimmed)
├── baselines/
...
```bash
# Proposed defense (ProtoKrum + ProtoTrimmed)
python ProtoKrum.py
...
python baselines/Median.py
python baselines/RFA.py
python baselines/Multi_Krum.py
```
