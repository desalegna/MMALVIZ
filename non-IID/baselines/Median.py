...
class Config:
    ...
    RESULTS_DIR       = os.environ.get("RESULTS_DIR", os.path.join(SCRIPT_DIR, "results_median_baseline"))
...
# ════════════════════════════════════════════════════════════════════════════
# Coordinate-wise Median aggregation (baseline — no prototype/cosine scoring)
# ════════════════════════════════════════════════════════════════════════════

def coordinate_wise_median(client_updates: List[Dict]) -> OrderedDict:
    """
    Coordinate-wise median aggregation (Yin et al., 2018).
    For each parameter tensor, takes the element-wise median across all clients.
    This is Byzantine-robust without any prototype or cosine-deviation scoring.
    """
    ...
    for key in keys:
        ...
        if stacked.dtype.is_floating_point:
            ...
            # For integer tensors (e.g. num_batches_tracked), use first client
            agg[key] = client_updates[0]['model_state'][key].clone()
    return agg


# ════════════════════════════════════════════════════════════════════════════
# Server — median baseline (no prototype / cosine deviation)
# ════════════════════════════════════════════════════════════════════════════
...
class MedianServer:
    ...
    Serves as the defence baseline against untargeted backdoor attacks.
    """
    ...
    def aggregate_models(self, client_updates: List[Dict]):
        ...
        print(f"  [MedianBaseline] Aggregated {len(client_updates)} clients "
...
def create_median_backdoor_system(model: nn.Module,
...
class EnhancedMetricsTracker:
    ...
    def plot_training_curves(self):
        ...
        plt.title(f'{self.n_shot}-shot Global Training Accuracy (Median Baseline)')
        ...
        plt.title(f'{self.n_shot}-shot Client Accuracies (Median Baseline)')
        plt.xlabel('Round'); plt.ylabel('Accuracy (%)')
        plt.grid(True, linestyle='--', alpha=0.7); plt.legend()
        plt.tight_layout()
        plt.savefig(os.path.join(
        ...
        plt.title(f'{self.n_shot}-shot Global Training Loss (Median Baseline)')
...
def save_experiment_config(config: Dict, results_dir: str):
    with open(os.path.join(results_dir, 'experiment_config.txt'), 'w') as f:
        f.write("Coordinate-wise Median Baseline Experiment Configuration\n")
        f.write("=" * 60 + "\n\n")
...
def plot_attack_metrics(attack_results: List[Dict], save_dir: str):
    ...
    ax1.set_title('Backdoor Misclassification Rate over Rounds (Median Baseline)')
    ...
    ax2.set_title('Clean Accuracy over Rounds (Median Baseline)')
    ax2.set_xlabel('Round'); ax2.set_ylabel('Accuracy (%)')
    ax2.grid(True, linestyle='--', alpha=0.7); ax2.legend()

    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'median_baseline_defense_metrics.png'))
    plt.close()
    ...
    plt.title('Per-Class Misclassification Rates (Median Baseline)')
...
def save_attack_results(attack_results: List[Dict], save_dir: str):
    path = os.path.join(save_dir, 'median_baseline_results.txt')
    with open(path, 'w') as f:
        f.write("Coordinate-wise Median Baseline Results\n")
        f.write("=" * 60 + "\n\n")
...
def train_median_baseline_model(server: MedianServer,
                                 num_rounds: int,
                                 local_epochs: int,
                                 results_dir: str,
                                 n_shot: int,
    ...
    print(f"\nStarting Coordinate-wise Median Baseline Federated Training:")
    print(f"N-shot: {n_shot}, N-way: {Config.N_WAY}, Query: {Config.N_QUERY}")
    print(f"Poisoning rate: {backdoor_cfg.POISONING_RATE}")
    print(f"Aggregation: Coordinate-wise Median (no prototype/cosine scoring)")
    print("=" * 80)
...
def main():
    ...
    print(f"Aggregation: Coordinate-wise Median (baseline — no prototype/cosine)")
    ...
    experiment_config = {
        ...
        'aggregation': {
            ...
            'reference':   'Yin et al., "Byzantine-Robust Distributed '
                           'Learning: Towards Optimal Statistical Rates", 2018'
        },
        'backdoor_config': {
            'type':            'Untargeted',
    ...
    try:
        ...
        for poison_rate in POISONING_RATES:
            for alpha in DIRICHLET_ALPHAS:
                ...
                # Directory layout: results/rate30/alpha0.2/1shot_median/
                ...
                for n_shot in [1, 5]:
                    ...
                    run_dir = os.path.join(combo_dir, f'{n_shot}shot_median')
                    ...
                    training_results = train_median_baseline_model(
                        server       = server,
                        num_rounds   = Config.NUM_ROUNDS,
                        local_epochs = Config.LOCAL_EPOCHS,
                        results_dir  = run_dir,
        ...
        with open(comparison_path, 'w') as f:
            f.write("Coordinate-wise Median Baseline Results Summary\n")
            f.write("=" * 80 + "\n\n")
        ...
        print("\n" + "="*70)
        print("FINAL RESULTS — Coordinate-wise Median Baseline")
        print("="*70)
...
def run_median_baseline_experiment():
    print("=" * 60)
    print("Coordinate-wise Median Baseline — Untargeted Backdoor Attacks")
    print("=" * 60)
    try:
