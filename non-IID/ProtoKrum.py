...
non-IID Dirichlet(alpha) client partition (alpha in {0.2, 0.5, 2.0})
...
class Config:
    ...
    RESULTS_DIR       = os.environ.get("RESULTS_DIR", os.path.join(SCRIPT_DIR, "results_ProtoKrum_defense"))


# Poisoning rates and Dirichlet alpha values to sweep
...
# ════════════════════════════════════════════════════════════════════════════
# Main  — Non-IID Dirichlet sweep over alpha ∈ {0.2, 0.5, 2.0}
# ════════════════════════════════════════════════════════════════════════════
    ...
    try:
        ...
        for poison_rate in POISONING_RATES:
            for alpha in DIRICHLET_ALPHAS:
                ...
                # Directory layout:  results/rate30/alpha0.2/1shot_protokrum/
                ...
                for n_shot in [1, 5]:
                    ...
                    # Held-out test dataset (test split only)
        ...
        # ── Summary table ─────────────────────────────────────────────────
        comparison_path = os.path.join(results_root,
                                       'final_protokrum_results.txt')
...
def run_protokrum_defense_experiment():
    ...
    print("Non-IID Dirichlet Partition  |  alpha ∈ {0.2, 0.5, 2.0}")
    print("=" * 60)
    try:
