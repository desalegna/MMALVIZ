...
# ════════════════════════════════════════════════════════════════════════════
# Prototype cosine deviation scoring  (Stage 1 — identical to ProtoKrum)
# ════════════════════════════════════════════════════════════════════════════
...
# ProtoTrimmed server
#
# Stage 1 (prototype cosine deviation -> per-client suspicion weight) is
# identical to ProtoKrum. Stage 2 replaces Krum selection with a weighted
...
#     4. Trim the n_trim clients with the smallest amp_i[d] (amplified-low
#        tail) and the n_trim clients with the largest amp_i[d]
#        (amplified-high tail).
#     5. Average the RAW (unweighted) v_i[d] values of the remaining
#        n - 2*n_trim clients.
#
# Suspicious clients (high Stage-1 weight) are pushed toward the trimmed
# tails even when their raw value isn't the most extreme one at that
# coordinate, while honest clients' values are combined with a plain mean
# (preserving trimmed mean's utility guarantees for the honest majority).
...
class ProtoTrimmedServer(Server):
    ...
    def aggregate_models(self, client_updates: List[Dict]):
        ...
        for key in keys:
            ...
            median_val = stacked.median(dim=0).values            # (*shape)
            w_shape    = [num_clients] + [1] * (stacked.dim() - 1)
            amp_dev    = (stacked - median_val.unsqueeze(0)) \
                * weights_cpu.view(w_shape)                       # amplified deviation

            order    = torch.argsort(amp_dev, dim=0)               # per-coordinate rank
            keep_idx = order[n_trim: n_trim + n_keep]               # drop both tails
...
# ════════════════════════════════════════════════════════════════════════════
# Deviation score plot  (Stage 1 — identical to ProtoKrum)
# ════════════════════════════════════════════════════════════════════════════

def plot_deviation_scores(server: ProtoTrimmedServer, save_dir: str, n_shot: int):
...
def plot_untargeted_attack_metrics(attack_results: List[Dict], save_dir: str):
    ...
    ax1.set_title('Backdoor Misclassification Rate over Rounds (ProtoTrimmed)')
    ...
    ax2.set_title('Clean Accuracy over Rounds (ProtoTrimmed)')
    ax2.set_xlabel('Round'); ax2.set_ylabel('Accuracy (%)')
    ax2.grid(True, linestyle='--', alpha=0.7); ax2.legend()

    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'prototrimmed_defense_metrics.png'))
    plt.close()
    ...
    plt.title('Per-Class Misclassification Rates (ProtoTrimmed)')
...
def save_untargeted_attack_results(attack_results: List[Dict],
                                   ...
                                   trim_fraction: float,
                                   save_dir: str):
    path = os.path.join(save_dir, 'prototrimmed_defense_results.txt')
    with open(path, 'w') as f:
        f.write("ProtoTrimmed Defense Results\n")
        f.write("=" * 60 + "\n\n")
...
def train_prototrimmed_model(server: ProtoTrimmedServer,
    ...
    print(f"\nStarting ProtoTrimmed Defense Federated Training:")
    ...
    save_untargeted_attack_results(
        attack_results, server.deviation_history, server.trim_fraction, results_dir)
...
def main():
    ...
    # trim_fraction=0.2 -> clips floor(5*0.2)=1 per tail, keeps 3 honest values
    TRIM_FRACTION = 0.2
    ...
    experiment_config = {
        ...
        'defense_method': {
            ...
            'stage_1':         'Prototype cosine deviation pre-weighting '
                               '(identical to ProtoKrum)',
            'stage_2':         'Weighted coordinate-wise trimmed mean: '
                               'Stage-1 weights amplify each client\'s '
                               'deviation from the per-coordinate median '
                               'before ranking; the most amplified-deviant '
                               'clients are trimmed from each tail, and the '
                               'raw values of the remaining clients are averaged.',
            'trim_fraction':   TRIM_FRACTION,
    ...
    try:
        ...
        all_results = {}  # keyed by (poison_rate, alpha)

        for poison_rate in POISONING_RATES:
            for alpha in DIRICHLET_ALPHAS:

                run_key = (poison_rate, alpha)
                print(f"\n{'#'*20}  rate={poison_rate}  alpha={alpha}  {'#'*20}")

                bdcfg = BackdoorConfig()
                bdcfg.POISONING_RATE = poison_rate

                # Directory layout: results/rate30/alpha0.2/1shot_prototrimmed/
                rate_tag  = f"rate{int(poison_rate * 100)}"
                alpha_tag = f"alpha{alpha}"
                combo_dir = os.path.join(results_root, rate_tag, alpha_tag)
                os.makedirs(combo_dir, exist_ok=True)

                # Partition once per (rate, alpha) combination;
                # both n_shot runs share the same partition for consistency.
        ...
        # ── Summary table ─────────────────────────────────────────────────
        comparison_path = os.path.join(results_root, 'final_prototrimmed_results.txt')
        with open(comparison_path, 'w') as f:
            ...
            f.write("Client partition: Non-IID Dirichlet\n")
            ...
            for poison_rate in POISONING_RATES:
                for alpha in DIRICHLET_ALPHAS:
                    for n_shot in [1, 5]:
                        ar   = all_results[(poison_rate, alpha)][n_shot][
                                   'attack_results']
                        last = ar[-1]
                        ...
                        avg_asr = np.mean([r['attack_success_rate']
                                           for r in ar])
                        f.write(
                            f"{poison_rate:<6} {alpha:<7} {n_shot:<6} "
        ...
        print("FINAL RESULTS")
        print("=" * 70)
        print(f"{'Rate':<6} {'Alpha':<7} {'Shot':<6} "
              f"{'ACC%':<10} {'ASR%':<10}")
        for poison_rate in POISONING_RATES:
            for alpha in DIRICHLET_ALPHAS:
                for n_shot in [1, 5]:
                    last = all_results[(poison_rate, alpha)][n_shot][
                               'attack_results'][-1]
