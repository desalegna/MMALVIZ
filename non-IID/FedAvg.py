...
def dirichlet_partition(
    ...
    is drawn.  This vector determines what fraction of that class's samples
    ...
    Every client is guaranteed at least one sample per class that has samples
    (replace=True fallback) so that few-shot episodes never starve.
    """
    ...
    for cls in classes:
        ...
        # Draw Dirichlet proportions and convert to sample counts
        proportions = rng.dirichlet(alpha=np.full(n_clients, alpha))
        counts      = (proportions * len(idx)).astype(int)

        # Distribute any leftover samples due to int truncation
        ...
        for cid, cnt in enumerate(counts):
            if cnt == 0:
                # Guarantee at least 1 sample so no client is class-blind
...
class EnhancedMetricsTracker:
        ...
        if class_performance:
            for class_name, mets in class_performance.items():
                ...
                for metric_name, value in mets.items():
                    self.class_metrics[class_name].setdefault(metric_name, []).append(value)
    ...
    def plot_confusion_matrix(self, class_names, final=True):
        if not self.confusion_matrices:
            return
        cm = self.confusion_matrices[-1] if final else np.mean(self.confusion_matrices, axis=0)
        num_classes = cm.shape[0]
        used = class_names[:num_classes]
        plt.figure(figsize=(10, 8))
        sns.heatmap(cm, annot=True, fmt='.2f', cmap='Blues',
                    xticklabels=used, yticklabels=used)
        label = "Final" if final else "Average"
        plt.title(f'{self.n_shot}-shot {label} Confusion Matrix (FedAvg — No Defense)')
    ...
    def save_detailed_results(self, class_names):
        path = os.path.join(self.results_dir, f'detailed_results_{self.n_shot}shot.txt')
    ...
    def plot_training_curves(self):
        ...
        plt.plot(rounds, self.metrics['accuracy'], 'b-', label='Global Accuracy', linewidth=2)
        plt.title(f'{self.n_shot}-shot Global Training Accuracy (FedAvg — No Defense)')
        plt.xlabel('Round')
        plt.ylabel('Accuracy (%)')
        plt.grid(True, linestyle='--', alpha=0.7)
        plt.legend()
        plt.tight_layout()
        plt.savefig(os.path.join(self.results_dir, f'global_accuracy_{self.n_shot}shot.png'))
        plt.close()

        plt.figure(figsize=(10, 6))
        for cid, accs in self.client_metrics.items():
            if accs:
                plt.plot(range(1, len(accs) + 1), accs, marker='o', markersize=4,
        ...
        plt.legend()
        plt.tight_layout()
        plt.savefig(os.path.join(self.results_dir, f'client_accuracies_{self.n_shot}shot.png'))
        plt.close()

        plt.figure(figsize=(10, 6))
        plt.plot(rounds, self.metrics['loss'], 'r-', label='Global Loss', linewidth=2)
        plt.title(f'{self.n_shot}-shot Global Training Loss (FedAvg — No Defense)')
        plt.xlabel('Round')
        plt.ylabel('Loss')
        plt.grid(True, linestyle='--', alpha=0.7)
        plt.legend()
        plt.tight_layout()
        plt.savefig(os.path.join(self.results_dir, f'global_loss_{self.n_shot}shot.png'))
        plt.close()
...
class Server:
    ...
    def aggregate_models(self, client_updates):
        """
        Standard weighted FedAvg: client model states are averaged with
        weights proportional to each client's average local training
        accuracy. No robustness mechanism (no clipping, no smoothing,
        no outlier rejection) is applied — this is the undefended baseline.
        """
        ...
        with torch.no_grad():
            for _ in range(n_ep):
                ...
                for i in range(len(unique_sl)):
                    mask = sl == i
                    proto_list.append(sf[mask].mean(0))
    ...
    def evaluate(self, test_dataset):
        ...
        try:
            cp, cm = calculate_class_metrics(preds, labels, test_dataset.class_names)
...
class UntargetedBackdoorServer(Server):
    ...
    def evaluate_untargeted_attack(self, test_dataset):
        ...
        try:
            clean_cm = confusion_matrix(cl_true, cp, labels=all_cls,
                                        normalize='true') * 100
        except Exception:
...
def calculate_class_metrics(predictions, labels, class_names):
    ...
    try:
        ...
        for i in cidx:
            ...
            if cn in rep:
                cp[cn] = {
                    'precision': rep[cn]['precision'],
                    'recall':    rep[cn]['recall'],
                    'f1_score':  rep[cn]['f1-score'],
                }
    except Exception:
...
def create_untargeted_backdoor_system(model, datasets, device, backdoor_config):
    ...
    for i, ds in enumerate(datasets):
        if i == backdoor_config.POISONED_CLIENT_ID:
            clients.append(UntargetedBackdoorClient(i, model, ds, device, backdoor_config))
        else:
            clients.append(FederatedClient(i, model, ds, device))
    return UntargetedBackdoorServer(model, clients, device, backdoor_config)


def plot_untargeted_attack_metrics(attack_results, save_dir):
    ...
    axes[0].set_title('Backdoor Misclassification Rate (FedAvg — No Defense)')
    ...
    axes[1].set_title('Clean Accuracy (FedAvg — No Defense)')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'untargeted_attack_metrics.png'))
    plt.close()
    ...
    for i, rn in enumerate(rounds):
        plt.annotate(
            str(rn),
            (attack_results[i]['clean_accuracy'], attack_results[i]['attack_success_rate']),
            xytext=(5, 5), textcoords='offset points',
        )
    plt.title('Attack Success vs. Clean Accuracy')
    plt.xlabel('Clean Accuracy (%)')
    plt.ylabel('Attack Success Rate (%)')
    plt.grid(True, linestyle='--', alpha=0.7)
    plt.colorbar(sc, label='Round')
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'attack_success_vs_clean_accuracy.png'))
    plt.close()


def save_untargeted_attack_results(attack_results, save_dir):
    path = os.path.join(save_dir, 'untargeted_attack_results.txt')
    with open(path, 'w') as f:
        f.write("Untargeted Backdoor Attack Results (FedAvg — No Defense)\n" + "="*60 + "\n\n")
        fm      = attack_results[-1]
...
def compare_pr_results(all_results, save_dir, pr_list, n_shots_list, alpha_list):
    ...
    for alpha in alpha_list:
        for n_shot in n_shots_list:
            ...
            for pr in pr_list:
                key = f'pr{pr}_ns{n_shot}_alpha{alpha}'
                if key not in all_results:
                    continue
                ar   = all_results[key]['attack_results']
                rnds = range(1, len(ar) + 1)
                ca   = [r['clean_accuracy']      for r in ar]
                asr  = [r['attack_success_rate'] for r in ar]
                axes[0].plot(rnds, ca,  marker='o', linewidth=2, label=f'PR={pr}')
                axes[1].plot(rnds, asr, marker='o', linewidth=2, label=f'PR={pr}')
            ...
            fig.suptitle(f'Poisoning Rate Comparison – {n_shot}-shot, '
                        f'alpha={alpha} (FedAvg — No Defense)', fontsize=14)
            plt.tight_layout()
            plt.savefig(os.path.join(
                cmp_dir, f'pr_comparison_{n_shot}shot_alpha{alpha}.png'))
            plt.close()
    ...
    with open(os.path.join(cmp_dir, 'pr_comparison_summary.txt'), 'w') as f:
        ...
        for alpha in alpha_list:
            for n_shot in n_shots_list:
                f.write(f"=== {n_shot}-shot | alpha={alpha} ===\n")
                for pr in pr_list:
                    key = f'pr{pr}_ns{n_shot}_alpha{alpha}'
                    if key not in all_results:
                        continue
                    ...
                    best_r = max(range(len(ar)), key=lambda i: ar[i]['attack_success_rate']) + 1
                    f.write(f"\n  PR={pr}\n")
...
def train_fedavg_model(server, num_rounds, local_epochs,
                       results_dir, n_shot, test_dataset):
    tracker    = EnhancedMetricsTracker(results_dir, n_shot)
    attack_dir = os.path.join(results_dir, f'{n_shot}shot_untargeted_backdoor')
    ...
    plot_untargeted_attack_metrics(attack_results, attack_dir)
    save_untargeted_attack_results(attack_results, attack_dir)
...
def write_final_summary(all_results: dict,
    ...
    Columns
    -------
    PR           – poisoning rate of the backdoor client
    Alpha        – Dirichlet concentration parameter (client heterogeneity)
    Shot         – few-shot k (support samples per class)
    ACC (final)  – global clean accuracy at the last FL round  (%)
    ASR (final)  – attack success rate at the last FL round    (%)
    Avg ACC      – clean accuracy averaged over all rounds     (%)
    Avg ASR      – attack success rate averaged over rounds    (%)
    """
    rows = []
    ...
    col_w = {"PR": 6, "Alpha": 7, "Shot": 6, "ACC (final)": 12,
             "ASR (final)": 12, "Avg ACC": 10, "Avg ASR": 10}
    cols  = ["PR", "Alpha", "Shot", "ACC (final)", "ASR (final)", "Avg ACC", "Avg ASR"]

    def fmt(col, val):
        if col == "PR" or col == "Alpha":
    ...
    lines += [
        ...
        "  Alpha       : Dirichlet concentration parameter for non-IID client partition",
        "                (lower = more heterogeneous; 2.0 = nearly IID)",
        "  Shot        : k-shot (number of support examples per class per episode)",
        "  ACC (final) : global clean accuracy evaluated at the final FL round  (%)",
        ...
        "  rejection is applied. Serves as the undefended baseline against the",
        "  robust aggregation rules (coordinate-wise median, Multi-Krum, RFA,",
        "  ProtoKrum, ProtoTrimmed).",
    ]
...
# ── Main ───────────────────────────────────────────────────────────────────────
def run_fedavg_baseline(n_shots_list=(1, 5), poisoning_rates=(0.3, 0.5),
                        dirichlet_alphas=(0.2, 0.5, 2.0)):
    ...
    print(f"Dirichlet alphas: {list(dirichlet_alphas)}")
    ...
    save_experiment_config({
        ...
        'client_partition':   'Non-IID Dirichlet — alpha in {0.2, 0.5, 2.0}',
        'aggregation': {
            ...
            'description': 'Standard weighted FedAvg, weighted by each '
                           ...
                           'is applied — undefended baseline.',
        },
        'backdoor': {
            'type':            'Untargeted',
    ...
    data_stats.plot_distribution(os.path.join(Config.RESULTS_DIR, 'data_distribution.png'))
    ...
    (tr_api, tr_trf, tr_lbl,
     te_api, te_trf, te_lbl) = split_data(
        api_imgs, trf_imgs, api_lbl, test_ratio=Config.TEST_SPLIT, seed=42)
    ...
    for pr in poisoning_rates:
        for alpha in dirichlet_alphas:
            ...
            print(f"\n\n{'#'*70}")
            print(f"#  Poisoning Rate = {pr}   |   Dirichlet alpha = {alpha}")
            print(f"{'#'*70}")

            cfg = BackdoorConfig()
            cfg.POISONING_RATE = pr

            # Directory layout: results/PR0.3/alpha0.2/1shot/
            combo_dir = os.path.join(
                Config.RESULTS_DIR, f'PR{pr}', f'alpha{alpha}')
            os.makedirs(combo_dir, exist_ok=True)

            # ── Non-IID Dirichlet partition of training data ────────────────
            partitions = dirichlet_partition(
                tr_api, tr_trf, tr_lbl,
                n_clients=Config.NUM_CLIENTS, alpha=alpha, seed=42)
            print_client_distribution(partitions, api_cls, alpha)
            plot_client_distribution(
                partitions, api_cls, alpha,
                save_path=os.path.join(
                    combo_dir, f'client_distribution_alpha{alpha}.png'))

            for n_shot in n_shots_list:
                tag     = f'pr{pr}_ns{n_shot}_alpha{alpha}'
                ...
                client_datasets = [
                    UntargetedBackdoorDataset(
                        p['api'], p['traffic'], p['labels'].copy(), api_cls, n_shot)
                    for p in partitions
                ]
    ...
    # ── Final summary table → final_fedavg_results.txt ────────────────────────
    write_final_summary(
...
def main():
    run_fedavg_baseline(n_shots_list=[1, 5], poisoning_rates=POISONING_RATES,
