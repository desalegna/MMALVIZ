...
Supports BOTH partition modes:
  • IID          — samples drawn uniformly at random across clients
  • Non-IID      — Dirichlet(alpha) partition, alpha ∈ {0.2, 0.5, 2.0}

Poisoning rates swept: {0.3, 0.5}
"""
...
PARTITION_MODES  = ["iid", "non-iid"]       # both run automatically
POISONING_RATES  = [0.3, 0.5]
DIRICHLET_ALPHAS = [0.2, 0.5, 2.0]          # only used for non-IID mode
...
# ════════════════════════════════════════════════════════════════════════════
# Prototype cosine deviation scoring
# ════════════════════════════════════════════════════════════════════════════
...
def split_data(
    ...
    print(f"\n[split_data] Total={len(labels)}  "
          f"Train={len(train_idx)}  Test={len(test_idx)}  [stratified]")
    ...
    return (api_images[train_idx],  traffic_images[train_idx], labels[train_idx],
...
def iid_partition(
    ...
    """
    IID partition: shuffle all samples and split into n_clients equal shards.
...
def dirichlet_partition(
    ...
    alpha ≈ 0.2  — highly heterogeneous (each client dominated by 1-2 classes)
    alpha ≈ 0.5  — moderately heterogeneous
    alpha ≈ 2.0  — nearly IID

    Every client is guaranteed ≥1 sample per class (replace=True fallback).
    """
    rng     = np.random.default_rng(seed)
...
def plot_client_distribution(partitions: List[Dict],
    ...
    for ci in range(n_classes):
        ...
        bottom += data[ci]
    ax.set_title(f'Per-client class distribution  ({title_label})')
...
class FederatedClient:
    ...
    def _train_episode(self, data) -> Tuple[float, float, Dict]:
        ...
        support_features = F.normalize(
            self.model(support_api, support_traffic), p=2, dim=1)
...
class UntargetedBackdoorClient(FederatedClient):
    ...
    def _train_episode(self, data) -> Tuple[float, float, Dict]:
        ...
        noise_loss = self.config.SCALE_FACTOR * F.mse_loss(
            query_features,
            random_direction.expand_as(query_features))
        noise_loss.backward()
        self.optimizer.step()
...
class Server:
    ...
    def evaluate(self, test_dataset: Dataset) -> Dict:
        ...
        for i, name in enumerate(test_dataset.class_names):
            pairs = [(p, t) for p, t in all_class_idx if t == i]
            class_accuracy[name] = (
...
class KrumServer(Server):
    ...
    def aggregate_models(self, client_updates: List[Dict]):
        ...
        # Stage 1: prototype cosine deviation
        if all('prototypes' in u for u in client_updates):
            ...
            weights = (1.0 + (dev_scores - mn) / (mx - mn)
                       if mx > mn
                       else torch.ones(num_clients, device=self.device))
            ...
            print("  [ProtoKrum] Cosine deviation: "
                  + ", ".join(f"C{i}={dev_scores[i]:.4f}"
                               for i in range(num_clients)))
            print("  [ProtoKrum] Amplification weights: "
        ...
        # Stage 2: amplified Krum scoring
        vecs = [
    ...
    def evaluate_untargeted_attack(self, test_dataset: Dataset,
        ...
        for cls in range(len(test_dataset.class_names)):
            ...
            class_misc[name] = (
                sum(1 for i in indices if bd_preds[i] != bd_lbls[i])
                / len(indices) * 100 if indices else 0.0)
...
def create_krum_backdoor_system(model: nn.Module,
    ...
    return KrumServer(model, clients, device, num_adversaries=num_adversaries)
...
class EnhancedMetricsTracker:
    ...
    def plot_training_curves(self):
        ...
        for metric_name, color, ylabel in [
            ('accuracy', 'b', 'Accuracy (%)'),
            ('loss',     'r', 'Loss'),
        ]:
            plt.figure(figsize=(10, 6))
            plt.plot(rounds, self.metrics[metric_name],
                     f'{color}-', label=f'Global {metric_name.capitalize()}',
                     linewidth=2)
            plt.title(f'{self.n_shot}-shot Global Training '
            ...
            plt.savefig(os.path.join(
                self.results_dir,
                f'global_{metric_name}_{self.n_shot}shot.png'))
            plt.close()
...
def load_images(directory: str, target_size: Tuple[int, int],
    ...
    if not os.path.exists(directory):
        raise FileNotFoundError(
            ...
            f"  Set API_IMAGE_DIR / TRAFFIC_IMAGE_DIR env vars, "
            f"or run from repository root.")
...
def align_traffic_with_api(api_data, traffic_data):
    ...
    skipped = 0
    for t_img, lbl, t_path in zip(traffic_images, traffic_labels,
                                   traffic_paths):
        lbl = int(lbl)
...
def plot_untargeted_attack_metrics(attack_results: List[Dict], save_dir: str,
    ...
    ax1.plot(rounds, [r['attack_success_rate'] for r in attack_results],
             'r-', linewidth=2)
    ax1.set_title(f'Misclassification Rate {tag}')
    ax1.set_xlabel('Round'); ax1.set_ylabel('Rate (%)')
    ax1.grid(True, linestyle='--', alpha=0.7)

    ax2.plot(rounds, [r['clean_accuracy'] for r in attack_results],
             'b-', linewidth=2)
    ax2.set_title(f'Clean Accuracy {tag}')
    ax2.set_xlabel('Round'); ax2.set_ylabel('Accuracy (%)')
    ax2.grid(True, linestyle='--', alpha=0.7)

    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f'protokrum_defense_metrics{tag}.png'))
    plt.close()

    if attack_results[0]['class_misclassification']:
        ...
        plt.title(f'Per-Class Misclassification {tag}')
        plt.xlabel('Round'); plt.ylabel('Rate (%)')
        plt.grid(True, linestyle='--', alpha=0.7)
        plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left')
        plt.tight_layout()
        plt.savefig(os.path.join(
            save_dir, f'per_class_misclassification{tag}.png'))
        plt.close()
...
def save_untargeted_attack_results(attack_results: List[Dict],
    ...
    with open(path, 'w') as f:
        ...
        for i, m in enumerate(attack_results, 1):
            f.write(f"\nRound {i}:\n"
                    f"  Clean Accuracy:    {m['clean_accuracy']:.2f}%\n"
                    f"  Misclassification: {m['attack_success_rate']:.2f}%\n")


# ════════════════════════════════════════════════════════════════════════════
# Training loop  (shared by IID and non-IID)
# ════════════════════════════════════════════════════════════════════════════
...
def train_untargeted_backdoor_model(server: KrumServer,
    ...
    print(f"\nStarting ProtoKrum Defense Federated Training {run_tag}:")
...
def run_iid_sweep(tr_api, tr_trf, tr_lbl,
                  te_api, te_trf, te_lbl,
                  api_classes, results_root,
                  device) -> Dict:
    """
    Sweep over poisoning rates × n_shot for the IID partition.
    Returns a dict keyed by (poison_rate, n_shot).
    """
    iid_results = {}
    for poison_rate in POISONING_RATES:
        ...
        iid_dir  = os.path.join(results_root, "iid", rate_tag)
        os.makedirs(iid_dir, exist_ok=True)

        print(f"\n{'#'*20}  [IID]  rate={poison_rate}  {'#'*20}")

        partitions = iid_partition(tr_api, tr_trf, tr_lbl,
                                   n_clients=Config.NUM_CLIENTS, seed=42)
        print_client_distribution(partitions, api_classes, "IID")
        plot_client_distribution(
            partitions, api_classes, "IID",
            save_path=os.path.join(iid_dir, 'client_distribution_iid.png'))

        for n_shot in [1, 5]:
            print(f"\n{'='*20} {n_shot}-shot | IID | rate={poison_rate} {'='*20}")
            run_dir = os.path.join(iid_dir, f'{n_shot}shot_protokrum')
            ...
            test_dataset = UntargetedBackdoorDataset(
                te_api, te_trf, te_lbl.copy(), api_classes, n_shot)

            model  = HybridNet().to(device)
            server = create_krum_backdoor_system(
                model=model, datasets=client_datasets, device=device,
                backdoor_config=bdcfg, num_adversaries=1)

            result = train_untargeted_backdoor_model(
                server=server, num_rounds=Config.NUM_ROUNDS,
                local_epochs=Config.LOCAL_EPOCHS, results_dir=run_dir,
                n_shot=n_shot, test_dataset=test_dataset,
                backdoor_cfg=bdcfg, run_tag=f"[IID rate={poison_rate}]")

            iid_results[(poison_rate, n_shot)] = result

    return iid_results


# ════════════════════════════════════════════════════════════════════════════
# Non-IID sweep helper
# ════════════════════════════════════════════════════════════════════════════

def run_noniid_sweep(tr_api, tr_trf, tr_lbl,
                     te_api, te_trf, te_lbl,
                     api_classes, results_root,
                     device) -> Dict:
    """
    Sweep over poisoning rates × Dirichlet alphas × n_shot.
    Returns a dict keyed by (poison_rate, alpha, n_shot).
    """
    noniid_results = {}
    for poison_rate in POISONING_RATES:
        for alpha in DIRICHLET_ALPHAS:
            ...
            combo_dir = os.path.join(results_root, "non_iid",
                                     rate_tag, alpha_tag)
            os.makedirs(combo_dir, exist_ok=True)

            print(f"\n{'#'*20}  [Non-IID]  rate={poison_rate}  alpha={alpha}  {'#'*20}")

            partitions = dirichlet_partition(
                tr_api, tr_trf, tr_lbl,
                n_clients=Config.NUM_CLIENTS, alpha=alpha, seed=42)
            print_client_distribution(
                partitions, api_classes, f"Dirichlet α={alpha}")
            plot_client_distribution(
                partitions, api_classes, f"Dirichlet α={alpha}",
            ...
            for n_shot in [1, 5]:
                print(f"\n{'='*20} {n_shot}-shot | Non-IID | "
                      f"rate={poison_rate} | alpha={alpha} {'='*20}")
                run_dir = os.path.join(combo_dir, f'{n_shot}shot_protokrum')
                os.makedirs(run_dir, exist_ok=True)
                ...
                result = train_untargeted_backdoor_model(
                    ...
                    n_shot=n_shot, test_dataset=test_dataset,
                    backdoor_cfg=bdcfg,
                    run_tag=f"[Non-IID rate={poison_rate} α={alpha}]")

                noniid_results[(poison_rate, alpha, n_shot)] = result

    return noniid_results
...
def write_iid_summary(iid_results: Dict, results_root: str):
    path = os.path.join(results_root, 'final_protokrum_iid_results.txt')
    with open(path, 'w') as f:
        f.write("ProtoKrum Defense Results Summary — IID Partition\n")
        f.write("=" * 70 + "\n\n")
        hdr = (f"{'Rate':<6} {'Shot':<6} {'ACC (final)':<14} "
               f"{'ASR (final)':<14} {'Avg ACC':<12} {'Avg ASR':<12}")
        f.write(hdr + "\n" + "-" * 70 + "\n")
        for poison_rate in POISONING_RATES:
            for n_shot in [1, 5]:
                ar   = iid_results[(poison_rate, n_shot)]['attack_results']
                last = ar[-1]
    ...
    print(f"\nIID summary written to: {path}")


def write_noniid_summary(noniid_results: Dict, results_root: str):
    path = os.path.join(results_root,
                        'final_protokrum_noniid_results.txt')
    with open(path, 'w') as f:
        f.write("ProtoKrum Defense Results Summary — Non-IID Partition\n")
        f.write("=" * 80 + "\n\n")
        hdr = (f"{'Rate':<6} {'Alpha':<7} {'Shot':<6} "
        ...
        for poison_rate in POISONING_RATES:
            for alpha in DIRICHLET_ALPHAS:
                for n_shot in [1, 5]:
                    ar   = noniid_results[
                        (poison_rate, alpha, n_shot)]['attack_results']
                    last = ar[-1]
                    avg_ca  = np.mean([r['clean_accuracy']      for r in ar])
                    avg_asr = np.mean([r['attack_success_rate'] for r in ar])
                    f.write(f"{poison_rate:<6} {alpha:<7} {n_shot:<6} "
                            f"{last['clean_accuracy']:<14.2f} "
                            f"{last['attack_success_rate']:<14.2f} "
                            f"{avg_ca:<12.2f} {avg_asr:<12.2f}\n")

    print(f"Non-IID summary written to: {path}")


def print_combined_summary(iid_results: Dict, noniid_results: Dict):
    print("\n" + "=" * 80)
    print("FINAL COMBINED RESULTS")
    print("=" * 80)

    print("\n── IID ──")
    print(f"{'Rate':<6} {'Shot':<6} {'ACC%':<10} {'ASR%':<10}")
    for poison_rate in POISONING_RATES:
        for n_shot in [1, 5]:
            last = iid_results[(poison_rate, n_shot)]['attack_results'][-1]
            print(f"{poison_rate:<6} {n_shot:<6} "
                  f"{last['clean_accuracy']:<10.2f} "
                  f"{last['attack_success_rate']:<10.2f}")

    print("\n── Non-IID ──")
    print(f"{'Rate':<6} {'Alpha':<7} {'Shot':<6} {'ACC%':<10} {'ASR%':<10}")
    for poison_rate in POISONING_RATES:
        for alpha in DIRICHLET_ALPHAS:
            for n_shot in [1, 5]:
                last = noniid_results[
                    (poison_rate, alpha, n_shot)]['attack_results'][-1]
                print(f"{poison_rate:<6} {alpha:<7} {n_shot:<6} "
                      f"{last['clean_accuracy']:<10.2f} "
                      f"{last['attack_success_rate']:<10.2f}")
...
def main():
    ...
    # ── IID sweep ─────────────────────────────────────────────────────────
    print("\n" + "█" * 30 + "  IID PARTITION  " + "█" * 30)
    iid_results = run_iid_sweep(
        tr_api, tr_trf, tr_lbl,
        te_api, te_trf, te_lbl,
        api_classes, results_root, device)

    # ── Non-IID sweep ─────────────────────────────────────────────────────
    print("\n" + "█" * 30 + "  NON-IID PARTITION  " + "█" * 30)
    noniid_results = run_noniid_sweep(
        tr_api, tr_trf, tr_lbl,
        te_api, te_trf, te_lbl,
        api_classes, results_root, device)

    # ── Write summaries ───────────────────────────────────────────────────
    write_iid_summary(iid_results, results_root)
    write_noniid_summary(noniid_results, results_root)
    print_combined_summary(iid_results, noniid_results)

    print(f"\nAll experiments completed. Results in: {results_root}")


def run_protokrum_defense_experiment():
    print("=" * 70)
    print("ProtoKrum Defense Against Untargeted Backdoor Attacks")
    print("Partition modes: IID  +  Non-IID Dirichlet α ∈ {0.2, 0.5, 2.0}")
    print("=" * 70)
    try:
...
if __name__ == '__main__':
    run_protokrum_defense_experiment()
