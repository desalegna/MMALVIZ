...
PARTITION_MODES below:
    ...
    PARTITION_MODES = ["iid"]              -> IID only
    PARTITION_MODES = ["noniid"]           -> Non-IID (Dirichlet) only
    PARTITION_MODES = ["iid", "noniid"]    -> both (default)
...
class Config:
    ...
    RESULTS_DIR = os.path.join(SCRIPT_DIR, "results_ProtoKrum_malimg")
...
def load_malimg(directory: str,
                image_size: Tuple[int, int]) -> Tuple[np.ndarray, np.ndarray, List[str]]:
    if not os.path.exists(directory):
        raise FileNotFoundError(
            f"Directory not found: {directory}\n"
            "Download MalImg from https://www.kaggle.com/datasets/manmandes/malimg "
            "and place it as described in the README, or set the "
            "MALIMG_DATASET_DIR environment variable."
        )
...
def dirichlet_partition(images: np.ndarray,
    ...
    """
    Partition data across clients using a Dirichlet distribution over class labels.

    For each class c, a proportion vector p ~ Dir(alpha) of length n_clients is
    drawn; samples of class c are then split according to p.  Smaller alpha
    produces more skewed (heterogeneous) distributions; larger alpha approaches
    IID (uniform) behaviour.
    ...
    min_samples_per_client : lower bound — clients below this threshold
                             receive a random top-up from the training pool
                             to avoid completely empty clients
    seed                   : RNG seed for reproducibility

    Returns
    -------
    List of dicts, each with keys 'images' and 'labels' (numpy arrays).
    """
    rng = np.random.default_rng(seed)
    classes = np.unique(labels)

    # Accumulate per-client index lists
    ...
    for cls in classes:
        ...
        # Convert proportions to split points (cumulative counts)
        n_cls = len(cls_idx)
        splits = (np.cumsum(proportions) * n_cls).astype(int)
        splits = np.clip(splits, 0, n_cls)
        splits[-1] = n_cls          # ensure all samples are assigned
    ...
    # Safety: top-up clients that received too few samples
...
def print_client_distribution(partitions: List[Dict],
                               ...
                               label: str = ""):
    """Print per-client class counts in a compact table."""
...
class UntargetedBackdoorDataset(Dataset):
    ...
    def __getitem__(self, index):
        # Only sample from classes that have enough data
        eligible = [
            cls for cls in self.categories
            if len(self.label_to_indices[cls]) >= self.n_support + self.n_query
        ]
        # Fall back to replacement sampling if too few eligible classes
...
class FederatedClient:
    ...
    def _compute_prototypes(self) -> Dict[int, torch.Tensor]:
        self.model.eval()
        proto_sums   = {}
        proto_counts = {}
        ...
        with torch.no_grad():
            for i in range(len(self.dataset.images)):
                img = self.dataset.images[i].unsqueeze(0).to(self.device)
                lbl = int(self.dataset.labels[i])
                emb = F.normalize(self.model(img), p=2, dim=1).squeeze(0)
                if lbl not in proto_sums:
                    proto_sums[lbl]   = emb.clone()
                    proto_counts[lbl] = 1
                else:
                    proto_sums[lbl]  += emb
                    proto_counts[lbl] += 1
        ...
        prototypes = {
            lbl: (proto_sums[lbl] / proto_counts[lbl]).cpu()
...
# ════════════════════════════════════════════════════════════════════════════
# ProtoKrum: Krum with prototype + cosine scoring
# ════════════════════════════════════════════════════════════════════════════

def flatten_model(state_dict: OrderedDict) -> torch.Tensor:
    parts = []
...
def prototype_cosine_scores(client_updates: List[Dict]) -> torch.Tensor:
    all_classes = set()
    for u in client_updates:
        all_classes.update(u['prototypes'].keys())

    global_protos = {}
    for cls in all_classes:
        vecs = [u['prototypes'][cls]
                for u in client_updates if cls in u['prototypes']]
        if vecs:
            global_protos[cls] = torch.stack(vecs).mean(0)

    n = len(client_updates)
    scores = torch.zeros(n)
    for i, u in enumerate(client_updates):
        sims = []
        for cls, g_proto in global_protos.items():
            if cls not in u['prototypes']:
                continue
            c_proto = u['prototypes'][cls]
            cos_sim = F.cosine_similarity(
                c_proto.unsqueeze(0), g_proto.unsqueeze(0)).item()
            sims.append(cos_sim)
...
def krum_with_prototype_cosine(client_updates: List[Dict],
                                n_byzantine: int = 1,
                                proto_weight: float = 0.5) -> OrderedDict:
    n = len(client_updates)

    proto_scores = prototype_cosine_scores(client_updates)

    n_neighbours = max(1, n - n_byzantine - 2)
    vectors      = [flatten_model(u['model_state']) for u in client_updates]
    ...
    for i in range(n):
        ...
        krum_scores[i]  = sorted_dists[:n_neighbours].sum()

    def minmax(t: torch.Tensor) -> torch.Tensor:
        lo, hi = t.min(), t.max()
        return (t - lo) / (hi - lo + 1e-8)

    norm_krum  = minmax(krum_scores)
    norm_proto = minmax(proto_scores)

    combined = (1.0 - proto_weight) * norm_krum + proto_weight * norm_proto

    winner_idx = int(combined.argmin().item())

    print(f"  [ProtoKrum] Combined scores: "
          f"{[f'{v:.3f}' for v in combined.tolist()]}  "
          f"→ selected client {winner_idx}")
    print(f"    Krum scores (norm):  {[f'{v:.3f}' for v in norm_krum.tolist()]}")
    print(f"    Proto-cos anomaly:   {[f'{v:.3f}' for v in norm_proto.tolist()]}")

    return copy.deepcopy(client_updates[winner_idx]['model_state'])
...
class KrumServer:
    ...
    def aggregate_models(self, client_updates: List[Dict]):
        new_state = krum_with_prototype_cosine(
            client_updates,
...
def create_krum_backdoor_system(model: nn.Module,
                                ...
                                n_byzantine: int = 1,
                                proto_weight: float = 0.5) -> KrumServer:
    clients = []
...
class EnhancedMetricsTracker:
    ...
    def plot_training_curves(self):
        ...
        plt.title(f'{self.n_shot}-shot α={self.alpha} '
                  f'Global Training Accuracy (ProtoKrum)')
        ...
        plt.title(f'{self.n_shot}-shot α={self.alpha} '
                  f'Global Training Loss (ProtoKrum)')
...
def save_experiment_config(config: Dict, results_dir: str):
    with open(os.path.join(results_dir, 'experiment_config.txt'), 'w') as f:
        f.write("ProtoKrum (Krum + Prototype/Cosine Scoring) — MalImg Dataset\n")
        f.write("=" * 60 + "\n\n")
...
def plot_attack_metrics(attack_results: List[Dict],
    ...
    ax1.set_title(f'Backdoor Misclassification Rate\n'
                  f'(ProtoKrum, {n_shot}-shot, α={alpha})')
...
def save_attack_results(attack_results: List[Dict],
    ...
    with open(path, 'w') as f:
        f.write(f"ProtoKrum (Krum + Prototype/Cosine Scoring) Results "
...
def train_krum_model(server: KrumServer,
    ...
    print(f"\nStarting ProtoKrum (Krum + Prototype/Cosine) Federated Training:")
    ...
    print(f"n_byzantine: {server.n_byzantine}, proto_weight: {server.proto_weight}")
    print("=" * 80)
    ...
    save_attack_results(attack_results, results_dir, n_shot, alpha)
...
def main():
    ...
    print(f"Aggregation:    ProtoKrum (Krum + Prototype/Cosine Scoring)")
    ...
    experiment_config = {
        ...
        'aggregation': {
            'method':       'Krum + Prototype/Cosine Scoring (ProtoKrum)',
            'n_byzantine':  N_BYZANTINE,
            'proto_weight': PROTO_WEIGHT,
            'description':  (
                'Standard Krum (Blanchard et al., 2017) extended with '
                'prototype-cosine deviation scoring. Combined score = '
                f'(1-{PROTO_WEIGHT})*norm(krum_dist) + {PROTO_WEIGHT}*norm(proto_anomaly). '
                'Single best client selected as new global model.'
            ),
            'references':   'Blanchard et al., NIPS 2017'
        },
        'backdoor_config': {
            'type':            'Untargeted',
    ...
    try:
            ...
            for alpha in alphas:
                ...
                for poison_rate in POISONING_RATES:
                    ...
                    for n_shot in [1, 5]:
                        ...
                        run_dir = os.path.join(rate_dir, f'{n_shot}shot_krum')
                        ...
                        server = create_krum_backdoor_system(
                            ...
                            n_byzantine=N_BYZANTINE, proto_weight=PROTO_WEIGHT)
        ...
        with open(summary_path, 'w') as f:
            f.write("ProtoKrum (Krum + Prototype/Cosine Scoring) — MalImg Summary\n")
            f.write("(IID + Non-IID)\n")
            f.write("=" * 70 + "\n\n")
        ...
        # ── Console summary ──────────────────────────────────────────────
        print("\n" + "=" * 70)
        print("FINAL RESULTS — ProtoKrum (MalImg, IID + Non-IID)")
        print("=" * 70)
...
def run_ProtoKrum_malimg_experiment():
    print("=" * 60)
    print("ProtoKrum — Untargeted Backdoor on MalImg (IID + Non-IID)")
    print("=" * 60)
    try:
...
if __name__ == '__main__':
    run_ProtoKrum_malimg_experiment()
