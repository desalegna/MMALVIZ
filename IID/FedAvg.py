"""
FedAvg Baseline — No Defense (Federated Learning).
...
def compare_pr_results(all_results, save_dir, pr_list, n_shots_list):
    ...
    for n_shot in n_shots_list:
        ...
        fig.suptitle(f'Poisoning Rate Comparison – {n_shot}-shot (FedAvg — No Defense)', fontsize=14)
        plt.tight_layout()
        plt.savefig(os.path.join(cmp_dir, f'pr_comparison_{n_shot}shot.png'))
        plt.close()
...
def write_final_summary(all_results: dict,
    ...
    col_w = {"PR": 6, "Shot": 6, "ACC (final)": 12,
             "ASR (final)": 12, "Avg ACC": 10, "Avg ASR": 10}
    cols  = ["PR", "Shot", "ACC (final)", "ASR (final)", "Avg ACC", "Avg ASR"]
...
def run_fedavg_baseline(n_shots_list=(1, 5), poisoning_rates=(0.3, 0.5)):
    ...
    for pr in poisoning_rates:
        for n_shot in n_shots_list:
            tag     = f'pr{pr}_ns{n_shot}'
            run_dir = os.path.join(Config.RESULTS_DIR, f'PR{pr}', f'{n_shot}shot')
            os.makedirs(run_dir, exist_ok=True)

            print(f"\n\n{'#'*70}")
            print(f"#  Poisoning Rate = {pr}   |   {n_shot}-shot")
            print(f"{'#'*70}")

            cfg = BackdoorConfig()
            cfg.POISONING_RATE = pr

            # ── IID partition of training data ─────────────────────────────
            partitions = iid_partition(tr_api, tr_trf, tr_lbl, Config.NUM_CLIENTS)
            print_client_distribution(partitions, api_cls)
