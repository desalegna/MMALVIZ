...
class UntargetedBackdoorClient(FederatedClient):
    ...
    def _train_episode(self, data) -> Tuple[float, float]:
        ...
        # Backdoor noise perturbation: push embeddings toward a random direction
        self.optimizer.zero_grad()
        rdir = F.normalize(
...
def coordinate_wise_median(client_updates: List[Dict]) -> OrderedDict:
    ...
    Unlike Multi-Krum or FoolsGold, this baseline does not score, select,
    or weight clients at all — every submitted parameter tensor
    contributes equally, and the global model takes the element-wise
    median across all client parameters. This makes it a useful "no
    similarity/no distance scoring" defence baseline: it is fairly robust
    to a *minority* of coordinate-wise outliers, but has no notion of a
    trusted subset the way Krum/Multi-Krum or FoolsGold do.
    """
    keys = client_updates[0]['model_state'].keys()
    agg  = OrderedDict()
    for key in keys:
        first_val = client_updates[0]['model_state'][key]
        if torch.is_tensor(first_val) and first_val.dtype.is_floating_point:
            ...
            # Integer buffers (e.g. BatchNorm's num_batches_tracked) are not
            # medianed — keep the first client's value as-is.
...
class MedianServer:
    ...
    def aggregate_models(self, client_updates: List[Dict]):
        ...
        print(f"  [MedianServer] Aggregated {len(client_updates)} clients "
              f"via coordinate-wise median (all clients contribute equally)")
...
def create_median_system(model: nn.Module,
...
def save_experiment_config(config: Dict, results_dir: str):
    with open(os.path.join(results_dir, 'experiment_config.txt'), 'w') as f:
        f.write("Coordinate-wise Median Baseline — MalImg Dataset\n")
        f.write("=" * 60 + "\n\n")
...
def plot_attack_metrics(attack_results: List[Dict], save_dir: str):
    ...
    ax1.set_title('Backdoor Misclassification Rate (Median Baseline)')
...
def train_median_baseline_model(server: MedianServer,
    ...
    print(f"Aggregation: Coordinate-wise Median (no scoring/selection)")
    print("=" * 80)
...
def main():
    ...
    print(f"Aggregation: Coordinate-wise Median (baseline, Yin et al., 2018)")
    ...
    experiment_config = {
        ...
        'aggregation': {
            ...
            'description': (
                'Element-wise median across all client parameters. '
                'No prototype scoring, no cosine/Krum distance, no '
                'client selection or weighting — every client '
                'contributes equally to each coordinate.'
            ),
    ...
    try:
        ...
        for partition_type, alpha in partition_combos:
            ...
            for poison_rate in POISONING_RATES:
                ...
                # Directory layout:
                #   results_median_baseline/iid/rate30/...
                #   results_median_baseline/noniid/alpha0_2/rate30/...
        ...
        with open(summary_path, 'w') as f:
            f.write("Coordinate-wise Median Baseline — MalImg IID vs Non-IID "
                    "Summary\n")
            f.write("=" * 70 + "\n\n")
            ...
            f.write(f"Aggregation: Coordinate-wise Median "
                    f"(no scoring/selection)\n")
...
def run_median_baseline_experiment():
    print("=" * 60)
    print("Coordinate-wise Median Baseline — Untargeted Backdoor on MalImg "
          "(IID + Non-IID)")
    print("=" * 60)
    try:
