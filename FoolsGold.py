"""
FoolsGold Aggregation  |  IID & Non-IID.
...
class Config:
    ...
    RESULTS_DIR       = os.environ.get("RESULTS_DIR",       os.path.join(SCRIPT_DIR, "results_foolsgold_iid_noniid"))
...
def iid_partition(
    ...
    uniformly at random — each client's local class distribution mirrors
    the global distribution (standard IID assumption).
    """
...
def print_iid_distribution(partitions: List[Dict], class_names: List[str]):
    ...
    for i in range(len(partitions)):
        print(f"  C{i+1:>3}", end='')
    print(f"  {'Total':>6}")
    print("-" * (12 + 7 * len(partitions) + 8))
    for ci, name in enumerate(class_names):
        print(f"{name:<12}", end='')
        row_total = 0
        for p in partitions:
            cnt = int(np.sum(p['labels'] == ci))
            row_total += cnt
            print(f"  {cnt:>4}", end='')
        print(f"  {row_total:>6}")
...
def plot_iid_distribution(partitions: List[Dict],
    ...
    data = np.zeros((n_clients, n_classes))
    for cid, p in enumerate(partitions):
        for ci in range(n_classes):
            data[cid, ci] = int(np.sum(p['labels'] == ci))
...
def dirichlet_partition(
    ...
    """
    Partition data among n_clients using Dirichlet(alpha) to simulate
    Non-IID heterogeneity.

    alpha interpretation:
      0.2  — high heterogeneity    (each client ≈ 1-2 classes)
      0.5  — moderate heterogeneity
      2.0  — mild heterogeneity   (close to IID)
      ∞    — IID
    """
    rng = np.random.default_rng(seed)
    ...
    for cls in classes:
        ...
        if remainder > 0:
            chosen = rng.choice(n_clients, remainder, replace=False)
            counts[chosen] += 1

        # Guarantee at least 1 sample per client per class
        for cid in range(n_clients):
            if counts[cid] == 0 and counts.max() > 1:
                donor = counts.argmax()
                counts[donor] -= 1
                counts[cid]   += 1
        ...
        for cid, cnt in enumerate(counts):
            ...
            shard = cls_idx[start:end]
            buckets[cid]['api'].extend(api_images[shard])
            buckets[cid]['traffic'].extend(traffic_images[shard])
            buckets[cid]['labels'].extend(labels[shard])
            start = end

    # Top-up any client below the minimum
    for cid in range(n_clients):
        while len(buckets[cid]['labels']) < min_samples_per_client:
            donor = max(
                (x for x in range(n_clients) if x != cid),
                key=lambda x: len(buckets[x]['labels']))
            if len(buckets[donor]['labels']) <= 1:
                break
            buckets[cid]['api'].append(buckets[donor]['api'].pop())
            buckets[cid]['traffic'].append(buckets[donor]['traffic'].pop())
            buckets[cid]['labels'].append(buckets[donor]['labels'].pop())
...
def plot_client_distribution(partitions: List[Dict],
    ...
    ax.set_xticks(range(n_clients))
    ax.set_xticklabels([f'C{i+1}' for i in range(n_clients)])
    ax.set_title(f'Non-IID Client Distribution (Dirichlet α={alpha})')
    ax.set_xlabel('Client')
    ax.set_ylabel('Number of Samples')
    ax.legend(bbox_to_anchor=(1.01, 1), loc='upper left', fontsize=8)
    plt.tight_layout()
...
def make_partitions(
    ...
    if mode == 'IID':
        ...
        return dirichlet_partition(api_images, traffic_images, labels,
                                   n_clients, alpha, seed=seed)
    else:
        raise ValueError(f"Unknown mode: {mode!r}. Use 'IID' or 'NonIID'.")
...
def flatten_model_update(model_state: OrderedDict) -> torch.Tensor:
    """Flatten all floating-point parameters into a 1-D vector."""
    return torch.cat([
        p.float().flatten()
...
def foolsgold_aggregate(
        ...
        contribution_history: np.ndarray,
        global_model_state:   OrderedDict,
        epsilon:              float = 1e-5,
) -> Tuple[OrderedDict, np.ndarray, np.ndarray]:
    """
    FoolsGold aggregation (Fung et al., 2018 / NDSS 2020).

    Clients with correlated cumulative gradient histories are down-weighted
    via cosine similarity + logit-based rescaling.  Clients with diverse
    updates retain full weight — Sybil/colluding clients converge to ≈ 0.

    Algorithm
    ---------
    1.  g_i = local_params_i − global_params
    2.  H  += g_i   (accumulate history)
    3.  S   = cosine-similarity matrix of normalised H rows
    4.  v_i = max_{j≠i} S[i, j]
    5.  α_i = 1 − v_i
    6.  α_i /= max(α)
    7.  lr_i = logit(clip(α_i, ε, 1−ε)), then min-max-scaled to [0, 1]
    8.  Weighted average of local parameters with normalised lr_i

    Returns
    -------
    aggregated_state      : new global model OrderedDict
    contribution_history  : updated history (n_clients, param_dim)
    client_weights        : per-client FoolsGold learning rates (n_clients,)
    """
    ...
    # 1. Compute gradients
    global_flat = flatten_model_update(global_model_state).cpu()
    gradients   = np.array([
        (flatten_model_update(u['model_state']).cpu() - global_flat).numpy()
        for u in client_updates
    ])

    # 2. Accumulate
    contribution_history += gradients

    # 3. Cosine similarity
    norms  = np.linalg.norm(contribution_history, axis=1, keepdims=True)
    norms  = np.where(norms < epsilon, epsilon, norms)
    H_norm = contribution_history / norms
    S      = H_norm @ H_norm.T

    # 4. Max similarity (excluding self)
    np.fill_diagonal(S, -1.0)
    v = S.max(axis=1)

    # 5-6. Forgetting factor, normalise
    alpha     = 1.0 - v
    max_alpha = alpha.max()
    if max_alpha > epsilon:
        alpha /= max_alpha

    # 7. Logit-based scaling
    alpha_c     = np.clip(alpha, epsilon, 1.0 - epsilon)
    logit_alpha = np.log(alpha_c / (1.0 - alpha_c))
    lo, hi      = logit_alpha.min(), logit_alpha.max()
    if hi - lo > epsilon:
        client_weights = (logit_alpha - lo) / (hi - lo)
    else:
        client_weights = np.ones(n_clients) / n_clients

    weight_sum = client_weights.sum()
    if weight_sum < epsilon:
        client_weights = np.ones(n_clients) / n_clients
        weight_sum     = 1.0

    print(f"  [FoolsGold] client weights: "
          + ", ".join(f"C{i}={w:.4f}" for i, w in enumerate(client_weights)))

    # 8. Weighted aggregation
    w_tensor = torch.tensor(
        client_weights / weight_sum,
        dtype=torch.float32, device=_ref_device)

    agg = OrderedDict()
    for key in client_updates[0]['model_state']:
        stacked = torch.stack(
            [u['model_state'][key].float() for u in client_updates], dim=0)
        if stacked.is_floating_point():
            wshape   = [-1] + [1] * (stacked.dim() - 1)
            agg[key] = (stacked * w_tensor.view(wshape)).sum(dim=0)
        else:
            agg[key] = client_updates[0]['model_state'][key].clone()

    return agg, contribution_history, client_weights
...
class FoolsGoldServer:
    """
    Federated server using FoolsGold aggregation.
    Reference: Fung et al., "Mitigating Sybils in Federated Learning
    Poisoning", NDSS 2020.
    """
    ...
    def __init__(self, model: nn.Module,
        ...
        self._contribution_history:   Optional[np.ndarray] = None
        self._client_weights_history: List[np.ndarray]     = []

    def _init_history(self, n_clients: int, param_dim: int):
        if self._contribution_history is None:
            self._contribution_history = np.zeros(
                (n_clients, param_dim), dtype=np.float64)

    def aggregate_models(self, client_updates: List[Dict]):
        n_clients = len(client_updates)
        param_dim = flatten_model_update(
            client_updates[0]['model_state']).numel()
        self._init_history(n_clients, param_dim)

        agg, self._contribution_history, weights = foolsgold_aggregate(
            client_updates       = client_updates,
            contribution_history = self._contribution_history,
...
def create_foolsgold_backdoor_system(
        model:           nn.Module,
        datasets:        List[Dataset],
        device:          torch.device,
        backdoor_config: BackdoorConfig) -> FoolsGoldServer:
    clients = []
    ...
    return FoolsGoldServer(model, clients, device)
...
class EnhancedMetricsTracker:
    ...
    def _safe_tag(self) -> str:
        return self.partition_label.replace('/', '_').replace(' ', '_')
    ...
    def plot_confusion_matrix(self, class_names: List[str], final: bool = True):
        ...
        plt.title(self._title_prefix()
                  + f'{"Final" if final else "Avg"} CM (FoolsGold)')
    ...
    def plot_training_curves(self):
        ...
        ax.set_title(title + 'Global Accuracy (FoolsGold)')
        ...
        ax.set_title(title + 'Client Accuracies (FoolsGold)')
        ax.set_xlabel('Round'); ax.set_ylabel('Accuracy (%)')
        ax.grid(True, ls='--', alpha=.7); ax.legend()
        plt.tight_layout()
        plt.savefig(os.path.join(self.results_dir,
        ...
        ax.set_title(title + 'Global Loss (FoolsGold)')
...
def save_experiment_config(config: Dict, results_dir: str):
    with open(os.path.join(results_dir, 'experiment_config.txt'), 'w') as f:
        f.write("FoolsGold — IID & Non-IID Experiment Configuration\n")
        f.write("=" * 60 + "\n\n")
...
def plot_attack_metrics(attack_results:  List[Dict],
    ...
    if attack_results and 'class_misclassification' in attack_results[0]:
        ...
        plt.title(title_prefix + 'Per-Class Misclassification (FoolsGold)')
    ...
    if weights_history:
        wh = np.array(weights_history)
        plt.figure(figsize=(12, 6))
        for cid in range(wh.shape[1]):
            plt.plot(rounds, wh[:, cid], marker='o', markersize=4,
                     label=f'Client {cid + 1}', linewidth=2)
        plt.title(title_prefix + 'FoolsGold Client Weights over Rounds')
        plt.xlabel('Round'); plt.ylabel('Weight')
        plt.grid(True, ls='--', alpha=.7); plt.legend()
        plt.tight_layout()
        plt.savefig(os.path.join(save_dir, 'foolsgold_client_weights.png'))
        plt.close()
...
def save_attack_results(attack_results:  List[Dict],
                         ...
                         save_dir:        str,
                         label:           str = ''):
    with open(os.path.join(save_dir, 'foolsgold_results.txt'), 'w') as f:
        f.write(f"FoolsGold Results  {label}\n")
        f.write("=" * 60 + "\n\n")
        ...
        if weights_history:
            f.write("\nFoolsGold Client Weights (Final Round):\n"
                    + "-" * 20 + "\n")
...
def train_foolsgold_model(
    ...
    plot_attack_metrics(attack_results, server.client_weights_history,
                         results_dir, partition_label + ' — ')
    save_attack_results(attack_results, server.client_weights_history,
                         results_dir, partition_label)
...
# ════════════════════════════════════════════════════════════════════════════
# Alpha / mode comparison plot
# ════════════════════════════════════════════════════════════════════════════

def plot_mode_comparison(all_results: Dict, results_root: str,
                          poison_rate: float, n_shot: int):
    """
    Line plots comparing clean accuracy and ASR across all partition
    modes (IID + all NonIID alphas) for a fixed (poison_rate, n_shot).
    """
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(15, 6))
    colors = ['#000000', '#e41a1c', '#377eb8', '#4daf4a']

    for color, (mode_label, _) in zip(
            colors,
            [('IID', None)] + [('NonIID', a) for a in DIRICHLET_ALPHAS]):

        key = (mode_label if mode_label == 'IID'
               else f'NonIID_α{_}', poison_rate, n_shot)
        if key not in all_results:
            continue
        ar     = all_results[key]['attack_results']
        rounds = range(1, len(ar) + 1)
        label  = key[0]

        ax1.plot(rounds, [r['clean_accuracy']      for r in ar],
                 color=color, linewidth=2, label=label)
        ax2.plot(rounds, [r['attack_success_rate'] for r in ar],
                 color=color, linewidth=2, label=label)

    ax1.set_title(f'Clean Accuracy — PR={poison_rate}, {n_shot}-shot')
    ax1.set_xlabel('Round'); ax1.set_ylabel('Accuracy (%)')
    ax1.grid(True, ls='--', alpha=.7); ax1.legend()

    ax2.set_title(f'ASR — PR={poison_rate}, {n_shot}-shot')
    ax2.set_xlabel('Round'); ax2.set_ylabel('ASR (%)')
    ax2.grid(True, ls='--', alpha=.7); ax2.legend()

    plt.suptitle(f'FoolsGold — IID vs Non-IID '
                 f'(PR={poison_rate}, {n_shot}-shot)', fontsize=13)
    plt.tight_layout()
    fname = (f'mode_comparison_PR{int(poison_rate*100)}_shot{n_shot}.png')
    plt.savefig(os.path.join(results_root, fname))
    plt.close()
...
def main():
    ...
    print(f"Aggregation: FoolsGold (similarity-based gradient down-weighting)")
    ...
    experiment_config = {
        ...
        'api_image_dir':      Config.API_IMAGE_DIR,
        'traffic_image_dir':  Config.TRAFFIC_IMAGE_DIR,        'api_image_size':     str(Config.API_IMAGE_SIZE),
        ...
        'aggregation': {
            'method':      'FoolsGold',
            'description': 'Down-weights clients with correlated gradient '
                           'histories via cosine similarity + logit scaling.',
            'reference':   'Fung et al., 2018 / NDSS 2020',
        },
        'backdoor_config': {
            'type':            'Untargeted',
    ...
    try:
        ...
        # ── Experiment loop ───────────────────────────────────────────────
        # key: (mode_label, poison_rate, n_shot)
        all_results: Dict[Tuple, Dict] = {}

        run_specs = [('IID', None)] + [('NonIID', a) for a in DIRICHLET_ALPHAS]
        ...
        for poison_rate in POISONING_RATES:
            ...
            for mode, alpha in run_specs:
                ...
                combo_dir  = os.path.join(rate_dir, mode_label)
                os.makedirs(combo_dir, exist_ok=True)

                print(f"\n{'═'*20}  {mode_label}  PR={poison_rate}  {'═'*20}")
                ...
                for n_shot in [1, 5]:
                    print(f"\n{'='*20} {n_shot}-shot | "
                          f"{mode_label} | PR={poison_rate} {'='*20}")

                    # Warn if any client lacks enough classes
                    for pid, p in enumerate(partitions):
                        n_cls = len(np.unique(p['labels']))
                        if n_cls < Config.N_WAY:
                            print(f"  [Warning] Client {pid} has only "
                                  f"{n_cls} classes (need {Config.N_WAY}).")

                    run_dir = os.path.join(
                        combo_dir, f'{n_shot}shot_foolsgold')
                    ...
                    server = create_foolsgold_backdoor_system(
                        model           = model,
                    ...
                    results = train_foolsgold_model(
                        ...
                        backdoor_cfg    = bdcfg,
                        partition_label = f'{mode_label} | PR={poison_rate}',
                    )
                    all_results[(mode_label, poison_rate, n_shot)] = results

            # Cross-mode comparison plots after each poison-rate block
            for n_shot in [1, 5]:
                try:
        ...
        # Final comparison plots (all combos complete)
        for poison_rate in POISONING_RATES:
            for n_shot in [1, 5]:
                try:
                    plot_mode_comparison(
                    ...
                    print(f"  [plot_mode_comparison] skipped: {e}")

        # ── Grand summary table ───────────────────────────────────────────
        summary_path = os.path.join(
            results_root, 'final_foolsgold_iid_noniid_results.txt')
        with open(summary_path, 'w') as f:
            f.write("FoolsGold Results Summary  (IID + Non-IID)\n")
            f.write("=" * 90 + "\n\n")
            f.write("Aggregation: FoolsGold\n")
        ...
        # Console summary
        print("\n" + "=" * 80)
        print("FINAL RESULTS — FoolsGold  (IID + Non-IID)")
        print("=" * 80)
...
def run_foolsgold_experiment():
    print("=" * 70)
    print("FoolsGold — Untargeted Backdoor Defence  |  IID & Non-IID")
    print("=" * 70)
    try:
