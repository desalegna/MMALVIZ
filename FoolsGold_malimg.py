...
class Config:
    ...
    RESULTS_DIR        = os.path.join(SCRIPT_DIR, "results_foolsgold")
...
class FoolsGoldConfig:
    """
    Hyper-parameters for the FoolsGold aggregation rule.

    FoolsGold (Fung et al., 2018) detects Sybil / colluding clients by
    tracking the cumulative gradient history of every client and penalising
    those whose contribution directions are suspiciously similar.

    ETA  – learning-rate dampening applied to the final per-client weights.
           Values in (0, 1]; 1.0 means no dampening beyond the cosine-
           similarity penalty.
    """
    ETA = 1.0   # dampening factor (κ in the original paper)
...
class FederatedClient:
    ...
    def train(self, global_model: nn.Module, local_epochs: int) -> Dict:
        ...
        # ── Flatten gradient update (Δw = w_local − w_global) ────────────
        global_vec = self._flatten_state(global_model.state_dict())
        local_vec  = self._flatten_state(self.model.state_dict())
        gradient   = local_vec - global_vec
...
class FoolsGold:
    """
    FoolsGold defence (Fung et al., 2018 / 2020).

    Core idea
    ---------
    Clients that submit highly similar gradient updates across rounds are
    likely colluding (Sybil attack).  FoolsGold maintains a cumulative
    gradient-history matrix H ∈ ℝ^{n_clients × d} and at every round:

      1. Appends the current pseudo-gradient to each client's history row.
      2. Computes pairwise cosine similarities among history rows.
      3. Derives a per-client learning-rate (trust weight) α_i ∈ (0, 1]
         that is dampened proportionally to the maximum similarity that
         client shares with any other client.
      4. Aggregates via a weighted average:
             w_global ← Σ_i α_i · w_i  /  Σ_i α_i
    """

    def __init__(self, n_clients: int, eta: float = 1.0):
        self.n       = n_clients
        self.eta     = eta
        self.history: torch.Tensor = None
        self.last_weights: np.ndarray = np.ones(n_clients) / n_clients

    def _init_history(self, d: int):
        self.history = torch.zeros(self.n, d, dtype=torch.float32)

    def _update_history(self, gradients: List[torch.Tensor]):
        for i, g in enumerate(gradients):
            self.history[i] += g.cpu().float()
    ...
    def _compute_weights(self) -> np.ndarray:
        ...
        cs     = torch.mm(H_norm, H_norm.t())
        cs.fill_diagonal_(0.0)
        max_sim, _ = cs.max(dim=1)

        alpha = torch.clamp(1.0 - max_sim, min=0.0, max=1.0)

        mx = alpha.max()
        if mx > 0:
            alpha = alpha / mx * self.eta

        alpha_np = alpha.numpy()

        if alpha_np.sum() == 0:
            alpha_np = np.ones(n) / n
        else:
            alpha_np = alpha_np / alpha_np.sum()

        self.last_weights = alpha_np
        return alpha_np

    def aggregate(self, client_updates: List[Dict],
                  global_model: nn.Module) -> Tuple[OrderedDict, np.ndarray]:
        gradients = [u['gradient'] for u in client_updates]
        d = gradients[0].shape[0]

        if self.history is None:
            self._init_history(d)

        self._update_history(gradients)
        ...
        for key in keys:
            ...
            if stacked.is_floating_point():
                ...
                vals = torch.stack(
                    [u['model_state'][key] for u in client_updates], dim=0)
                agg[key] = vals.float().mean(0).long()
...
class FoolsGoldServer:
    def __init__(self, model: nn.Module,
                 ...
                 device: torch.device,
                 fg_config: FoolsGoldConfig = None):
        self.global_model  = model
        self.clients       = clients
        self.device        = device
        self.fg            = FoolsGold(
            n_clients = len(clients),
            eta       = (fg_config.ETA if fg_config else 1.0),
        )
        self.attack_metrics: List[Dict] = []
        self.trust_history:  List[np.ndarray] = []

    def aggregate_models(self, client_updates: List[Dict]):
        agg, weights = self.fg.aggregate(client_updates, self.global_model)
        ...
        print(f"  [FoolsGold] Weights: {weight_str}")
...
def create_foolsgold_system(model: nn.Module,
                             datasets: List[UntargetedBackdoorDataset],
                             device: torch.device,
                             backdoor_config: BackdoorConfig,
                             fg_config: FoolsGoldConfig) -> FoolsGoldServer:
    clients = []
    ...
    return FoolsGoldServer(model, clients, device, fg_config)
...
class EnhancedMetricsTracker:
    ...
    def plot_training_curves(self):
        ...
        plt.title(f'{self.n_shot}-shot Global Training Accuracy (FoolsGold)')
        ...
        plt.title(f'{self.n_shot}-shot Client Accuracies (FoolsGold)')
        plt.xlabel('Round'); plt.ylabel('Accuracy (%)')
        plt.grid(True, linestyle='--', alpha=0.7); plt.legend()
        plt.tight_layout()
        plt.savefig(os.path.join(
        ...
        plt.title(f'{self.n_shot}-shot Global Training Loss (FoolsGold)')
    ...
    def plot_trust_weights(self, n_clients: int, poisoned_id: int):
        ...
        plt.title(f'{self.n_shot}-shot FoolsGold Trust Weights over Rounds')
        plt.xlabel('Round')
        plt.ylabel('Trust Weight (α)')
        plt.ylim(-0.02, 1.05)
        ...
        plt.savefig(os.path.join(
            self.results_dir,
            f'foolsgold_trust_weights_{self.n_shot}shot.png'), dpi=150)
        plt.close()
...
def save_experiment_config(config: Dict, results_dir: str):
    with open(os.path.join(results_dir, 'experiment_config.txt'), 'w') as f:
        f.write("FoolsGold Defence — MalImg Dataset\n")
        f.write("=" * 60 + "\n\n")
...
def plot_attack_metrics(attack_results: List[Dict], save_dir: str):
    ...
    ax1.set_title('Backdoor Misclassification Rate (FoolsGold)')
    ...
    ax2.set_title('Clean Accuracy over Rounds (FoolsGold)')
    ax2.set_xlabel('Round'); ax2.set_ylabel('Accuracy (%)')
    ax2.grid(True, linestyle='--', alpha=0.7); ax2.legend()

    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'foolsgold_defense_metrics.png'), dpi=150)
    plt.close()
    ...
    if attack_results and attack_results[0]['class_misclassification']:
        ...
        plt.title('Per-Class Misclassification Rates (FoolsGold)')
...
def save_attack_results(attack_results: List[Dict], save_dir: str):
    path = os.path.join(save_dir, 'foolsgold_results.txt')
    with open(path, 'w') as f:
        f.write("FoolsGold Defence Results\n")
        f.write("=" * 60 + "\n\n")
...
def train_foolsgold_model(server: FoolsGoldServer,
                           num_rounds: int,
                           local_epochs: int,
                           results_dir: str,
                           n_shot: int,
...
def main():
    ...
    print(f"Defence: FoolsGold (Fung et al., 2018)")
    ...
    experiment_config = {
        ...
        'aggregation': {
            ...
            'eta':         fg_cfg.ETA,
            'description': (
                'Similarity-based per-client trust weighting. '
                'Clients with high gradient-history cosine similarity '
                'receive dampened aggregation weights.'
            ),
            'reference':   (
                'Fung et al., "Mitigating Sybils in Federated Learning '
                'Poisoning", 2018 / NDSS 2021'
            ),
        },
        'backdoor_config': {
            'type':            'Untargeted',
    ...
    try:
        ...
        for partition_type, alpha in partition_combos:
            ...
            for poison_rate in POISONING_RATES:
                ...
                # Directory layout:
                #   results_foolsgold/iid/rate30/...
                #   results_foolsgold/noniid/alpha0_2/rate30/...
                ...
                for n_shot in [1, 5]:
                    ...
                    server = create_foolsgold_system(
                        model           = model,
                    ...
                    training_results = train_foolsgold_model(
                        server       = server,
                        num_rounds   = Config.NUM_ROUNDS,
                        local_epochs = Config.LOCAL_EPOCHS,
                        results_dir  = run_dir,
        ...
        # ── Summary table ─────────────────────────────────────────────────
        summary_path = os.path.join(results_root, 'final_foolsgold_results.txt')
        with open(summary_path, 'w') as f:
            f.write("FoolsGold Defence — MalImg IID vs Non-IID Summary\n")
            f.write("=" * 70 + "\n\n")
            ...
            f.write(f"{'Partition':<10} {'PR':<6} {'Shot':<6} "
            ...
            for partition_type, alpha in partition_combos:
                ...
                for pr in POISONING_RATES:
                    for ns in [1, 5]:
                        ...
                        f.write(f"{part_label:<10} {pr:<6} {ns:<6} "
        ...
        # ── Console summary ───────────────────────────────────────────────
        print("\n" + "=" * 70)
        print("FINAL RESULTS — FoolsGold Defence (MalImg, IID vs Non-IID)")
        print("=" * 70)
        print(f"{'Partition':<12} {'PR':<6} {'Shot':<6} {'ACC%':<10} {'ASR%':<10}")
        for partition_type, alpha in partition_combos:
            part_label = 'IID' if partition_type == 'iid' \
                         else f'noniid-a{alpha}'
            for pr in POISONING_RATES:
                for ns in [1, 5]:
                    ...
                    print(f"{part_label:<12} {pr:<6} {ns:<6} "
...
def run_foolsgold_experiment():
    print("=" * 60)
    print("FoolsGold Defence — Untargeted Backdoor on MalImg (IID + Non-IID)")
    print("=" * 60)
    try:
