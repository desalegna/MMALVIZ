"""
Multi-Krum Aggregation — IID & Non-IID Baseline.
...
def iid_partition(
    ...
    # Split shuffled indices into n_clients roughly-equal chunks
...
def print_iid_distribution(partitions: List[Dict],
...
def multi_krum_aggregate(client_updates: List[Dict],
                          n_byzantine: int = 1,
                          m: Optional[int] = None) -> OrderedDict:
    """
    Multi-Krum aggregation (Blanchard et al., NeurIPS 2017).

    Selects the m clients with the lowest Krum scores (sum of squared L2
    distances to each client's nearest n−f−2 neighbours) and averages their
    parameters with equal weight.  m=1 recovers standard Krum.
    ...
    if n < 2 * n_byzantine + 3:
        raise ValueError(
            f"Multi-Krum requires n >= 2f+3. Got n={n}, f={n_byzantine}.")
    ...
    # Flatten parameters
    flat = [torch.cat([p.float().flatten()
    ...
    print(f"  [Multi-Krum] Selected: {selected}")

    states = [client_updates[i]['model_state'] for i in selected]
    agg    = OrderedDict()
    for key in states[0]:
        agg[key] = torch.stack([s[key].float() for s in states]).mean(0)
    return agg


def krum_aggregate(client_updates: List[Dict],
                   n_byzantine: int = 1) -> OrderedDict:
    """Standard Krum (m=1 special case of Multi-Krum)."""
...
class KrumServer:
    """
    Federated server using Multi-Krum aggregation.
    ...
    @staticmethod
    def _run_episodes(model, loader, device, n_episodes) -> Tuple[List, List]:
        preds, labels = [], []
        with torch.no_grad():
...
def create_krum_backdoor_system(
        model:           nn.Module,
        datasets:        List[Dataset],
        device:          torch.device,
        backdoor_config: BackdoorConfig,
        n_byzantine:     int = 1,
        multi_krum_m:    Optional[int] = None) -> KrumServer:
    clients = []
...
class EnhancedMetricsTracker:
    ...
    def plot_confusion_matrix(self, class_names: List[str], final: bool = True):
        ...
        plt.title(self._title_prefix()
                  + f'{"Final" if final else "Avg"} CM (Multi-Krum)')
    ...
    def plot_training_curves(self):
        ...
        ax.set_title(title + 'Global Accuracy (Multi-Krum)')
        ...
        ax.set_title(title + 'Client Accuracies (Multi-Krum)')
        ax.set_xlabel('Round'); ax.set_ylabel('Accuracy (%)')
        ax.grid(True, ls='--', alpha=.7); ax.legend()
        plt.tight_layout()
        plt.savefig(os.path.join(self.results_dir,
        ...
        ax.set_title(title + 'Global Loss (Multi-Krum)')
...
def plot_attack_metrics(attack_results: List[Dict], save_dir: str,
    ...
    if attack_results and 'class_misclassification' in attack_results[0]:
        ...
        plt.title(title_prefix + 'Per-Class Misclassification (Multi-Krum)')
...
def save_attack_results(attack_results: List[Dict],
    ...
    with open(os.path.join(save_dir, 'multikrum_results.txt'), 'w') as f:
        f.write(f"Multi-Krum Results  {label}\n")
        f.write("=" * 60 + "\n\n")
...
def train_krum_model(server:       KrumServer,
    ...
    plot_attack_metrics(attack_results, results_dir, partition_label + ' — ')
...
def main():
    ...
    print(f"Aggregation: Multi-Krum")
    ...
    # n >= 2f+3  ⟹  with 5 clients max f = 1
    N_BYZANTINE  = 1
    MULTI_KRUM_M = Config.NUM_CLIENTS - N_BYZANTINE  # = 4
    ...
    try:
        ...
        # ── Experiment loop ───────────────────────────────────────────────
        # We iterate over partition modes and collect all results.
        # IID runs once; NonIID runs for each alpha value.

        all_results = {}  # key: (mode_label, poison_rate, n_shot)

        # Build list of (partition_mode, alpha_or_None) combos
        run_specs = [('IID', None)]
        ...
        for poison_rate in POISONING_RATES:
            for mode, alpha in run_specs:
                ...
                # Human-readable label e.g. "IID" or "NonIID_α0.2"
                ...
                for n_shot in [1, 5]:
                    ...
                    run_dir = os.path.join(
                        combo_dir, f'{n_shot}shot_multikrum')
        ...
        with open(summary_path, 'w') as f:
            f.write("Multi-Krum Results Summary  (IID + Non-IID)\n")
            f.write("=" * 90 + "\n\n")
        ...
        # ── Console summary ────────────────────────────────────────────────
        print("\n" + "=" * 80)
        print("FINAL RESULTS — Multi-Krum  (IID + Non-IID)")
        print("=" * 80)
...
def run_krum_experiment():
    print("=" * 70)
    print("Multi-Krum — Untargeted Backdoor Defence  |  IID & Non-IID")
    print("=" * 70)
    try:
