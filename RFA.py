"""
RFA — Robust Federated Aggregation  |  IID & Non-IID Baseline.
...
class Config:
    ...
    RESULTS_DIR       = os.environ.get(
        "RESULTS_DIR",       os.path.join(SCRIPT_DIR, "results_rfa"))
...
def iid_partition(
    ...
    uniformly at random, so each client's local class distribution mirrors
    the global distribution — the standard IID assumption.
    """
...
def rfa_aggregate(
    ...
    """
    RFA aggregation (Pillutla et al., IEEE Trans. Signal Processing, 2022).
    ...
    Parameters
    ----------
    nu      : smoothing constant (default 0.1)
    T       : Weiszfeld iterations (default 8)
    ...
    """
    # Device of the first floating-point parameter
    _ref_device = next(
    ...
    ], dtype=np.float64)   # (n_clients, param_dim)
    ...
    for _ in range(T):
        ...
        mu      = (w[:, None] * params).sum(axis=0) / w.sum()
        if np.linalg.norm(mu - mu_prev) < epsilon:
            break

    # Final normalised weights for logging / plotting
    ...
    # Reconstruct aggregated OrderedDict (back on _ref_device)
    agg       = OrderedDict()
    fp_offset = 0
...
class RFAServer:
    ...
    def aggregate_models(self, client_updates: List[Dict]):
        agg, weights = rfa_aggregate(client_updates, nu=self.nu, T=self.T)
...
def create_rfa_backdoor_system(
        model:           nn.Module,
        datasets:        List[Dataset],
        device:          torch.device,
        backdoor_config: BackdoorConfig) -> RFAServer:
    clients = []
...
class EnhancedMetricsTracker:
    ...
    def plot_confusion_matrix(self, class_names: List[str], final: bool = True):
        ...
        plt.title(self._title_prefix()
                  + f'{"Final" if final else "Avg"} CM (RFA)')
    ...
    def plot_training_curves(self):
        ...
        ax.set_title(title + 'Global Accuracy (RFA)')
        ...
        ax.set_title(title + 'Client Accuracies (RFA)')
        ax.set_xlabel('Round'); ax.set_ylabel('Accuracy (%)')
        ax.grid(True, ls='--', alpha=.7); ax.legend()
        plt.tight_layout()
        plt.savefig(os.path.join(self.results_dir,
        ...
        ax.set_title(title + 'Global Loss (RFA)')
...
def save_experiment_config(config: Dict, results_dir: str):
    with open(os.path.join(results_dir, 'experiment_config.txt'), 'w') as f:
        f.write("RFA — IID & Non-IID Experiment Configuration\n")
        f.write("=" * 60 + "\n\n")
...
def plot_attack_metrics(attack_results:  List[Dict],
    ...
    if attack_results and 'class_misclassification' in attack_results[0]:
        ...
        plt.title(title_prefix + 'Per-Class Misclassification (RFA)')
    ...
    if weights_history:
        ...
        for cid in range(wh.shape[1]):
            plt.plot(rounds, wh[:, cid], marker='o', markersize=4,
                     label=f'Client {cid + 1}', linewidth=2)
        plt.title(title_prefix + 'RFA Client Weights over Rounds')
...
def save_attack_results(attack_results:  List[Dict],
                         ...
                         save_dir:        str,
                         label:           str = ''):
    with open(os.path.join(save_dir, 'rfa_results.txt'), 'w') as f:
        f.write(f"RFA Results  {label}\n")
        f.write("=" * 60 + "\n\n")
...
def main():
    ...
    try:
        ...
        for poison_rate in POISONING_RATES:
            for mode, alpha in run_specs:
                ...
                for n_shot in [1, 5]:
                    ...
                    run_dir = os.path.join(combo_dir, f'{n_shot}shot_rfa')
        ...
        # ── Summary table ─────────────────────────────────────────────────
        summary_path = os.path.join(results_root, 'final_rfa_results.txt')
        with open(summary_path, 'w') as f:
            f.write("RFA Results Summary  (IID + Non-IID)\n")
            f.write("=" * 90 + "\n\n")
        ...
        # ── Console summary ────────────────────────────────────────────────
        print("\n" + "=" * 80)
        print("FINAL RESULTS — RFA  (IID + Non-IID)")
        print("=" * 80)
